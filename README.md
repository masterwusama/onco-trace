...
危险度这一维匿名侧只有骨架、没有强度。GBD 的比较风险评估数值与统计层撞在同一道 IHME 注册门后
——vizhub GBD Compare 的四个数据接口一律 401，匿名开放的只有回版本号的那一个。能匿名取回的是
随 GBD 发布的 A2 交叉表：2390 对病因 × 危险因素关联（217 个病因 × 88 个危险因素），按 REI 层级
剔掉聚合档后 18 病里 **10 病有 ≥3 个独立危险因素**（肺癌 16、结直肠 11、乳腺 7，脑肿瘤一档都没有）。
但表里 Deaths/YLLs/YLDs/DALYs 四列只是"这个组合有数"的标记，没有一个 PAF 或 RR——
反查可以按关联建，危险度页面不做归因强度榜。
