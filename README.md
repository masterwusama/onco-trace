...
具体几个随网络状况变，以最近一次 `probe-reach` 为准）。已过出口判据的四维：
...
- **前沿研究**——ClinicalTrials.gov v2、Europe PMC、Open Targets 三源各 18/18，是 P0 唯一
  三列判据全过的一批。疾病键只有 Open Targets 能用 ID（MONDO 号点查全中），
  另两源只能按逐病声明的查询词表，实测数字与口径见下。
...
Open Targets 的关联靶点最少也有 643 个（判据是 10）。这一维真正的产出是疾病键的结论：
...
按记录级抽样只有 31.7% 能真读到正文，症状抽取要看后者；CT 匿名侧没有地理过滤器
...
点查够用，不下载整包；但它把乳腺与胰腺落在窄档（自身文献 88 / 382 而父节点 710,750 / 166,685），
"这一病的研究热度"要按父节点重新声明才算得对。
