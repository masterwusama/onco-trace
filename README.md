...
`python api/tests/run.py` 现在 4,202 条断言、整跑 27 s：四台仍是两路对照，聚法那台两侧算法刻意
...
│ probe*       覆盖度探针           │         │ routes/  meta/疾病/统计/生存/词表  │
│ load/        探针解析 → 业务表行  │         └────────────────────────────────────┘
...
python api/tests/run.py                  # 服务层对账：十条接口返回的每个数字另问一次 SQL（要连着库）
