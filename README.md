...
19 个源全部可达（17 个直连、2 个需代理）。已过出口判据的三维：
...
统计层部分达标：发病率与死亡率的年度序列 1975–2024 逐年齐备（死亡 50 年、发病 49 年），
另有 6 组种族/民族与双性别分层可用；但 SEER 的年龄分组只有 8 档宽区间（`<20` 到 `>84`），
做不了 5 岁组标化率。唯一按 5 岁组乃至 1 岁组发布的 GBD，**数值全部在注册登录门后**——
词表实测 18 病 18/18 有对应病因档、中国在内、1990–2021 逐年、155 个年龄组、四个度量齐备，
可一个数都取不回来（2023 版 22 个数据集的 118 个文件全指向登录页）。
5 岁组这一档现在只剩 GLOBOCAN 与 WHO GHO 两条匿名路。
