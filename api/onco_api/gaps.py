...
裁定在 docs/MVP裁定.md §二，那四处是 P0 实测出来的结论。这里不照抄那份名单，
而是把每一处写成"读哪个度量、等于几才算缺"，所以：

- 装载器哪天把 `freq_band` 填上了，接口就不再说它空着——文档会漂，谓词不会；
  这一条已经实际发生过一次：`paf` 填满 71 行后（2026-09-10），它的空态连同谓词一起删了；
...
DISEASE_RULES: tuple[Rule, ...] = (
    (
        ...
        "三条匿名中文路实测只覆盖 7/18 病，低于裁定线；按源分行而不是做翻译列（§二.2）",
        lambda x: x["counts"]["symptom"]["zh"] == 0,
    ),
    (
        "symptom",
        "症状频率带空缺：PDQ 的症状小节里没有一条百分号。",
        "唯一给得出六档的 Orphanet 在常见上皮癌上 0 命中，所以 freq_band 建而不填（§二.1）",
    ...
    (
        ...
        "也不与遗传关联排成同一张榜。",
        "CRA 按 REI 层级剔掉聚合父档后 10/18 病达到 ≥3 个独立暴露（§一 危险因素清单行）",
        lambda x: 0 < x["counts"]["risk"]["exposure"] < 3,
    ),
    (
        "survival",
        "这一病没有分期别的五年生存率：源本身不发分期表，只有全分期一个数。",
        "SEER 对白血病整页无分期表（§一 五年存活率行、§二.4）",
