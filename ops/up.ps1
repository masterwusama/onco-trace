...
#
# Before starting anything this script checks three things, all read-only: the .env settings
# (via api/onco_api/config.py, so there is only ever one parser of that file), that MySQL is
# actually listening, and that the ports it is about to use are free. The last one matters:
# a backend still running with yesterday's code answers requests fine and shows numbers that
# no longer match the repository, which looks like a data bug and is not one.
#
# What it deliberately does NOT do: apply migrations (a schema change does not belong in
# "start" -- run python db/tests/run.py migrate), kill whatever holds a port, or run the gates.
...
function Require-FreePort([int]$port, [string]$what) {
    $conn = Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue |
        Select-Object -First 1
    if ($conn) {
        $proc = Get-Process -Id $conn.OwningProcess -ErrorAction SilentlyContinue
        $name = if ($proc) { $proc.ProcessName } else { '?' }
        $held = ('{0} port {1} is held by PID {2} ({3}). Stop that process yourself first -- ' +
                 'this script never kills what it did not start.') -f $what, $port, $conn.OwningProcess, $name
        Die $held
    }
...
Require-FreePort ([int]$apiPort) 'backend'

if ($mode -eq 'dev') {
    # vite already refuses a taken port (strictPort); naming the holder here is the readable
    # version of that same refusal
    Require-FreePort $vitePort 'vite dev'
    Write-Host ''
