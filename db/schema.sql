...
--
-- 前四张是"源与探针"表（P0），后面十五张是业务表（P1 的 C1c），两段合起来是全量终态。
-- 业务表刻意留到 P0 之后才建：P0 的任务是先实测每一维到底有没有可自动落库的源。
-- 症状维已经踩过一次——HPO/Orphanet/NCIt 的文档都声称有疾病↔症状注释，
-- 实测常见上皮癌只有 3~9 行且全是肿瘤同义复述。先按想象建 20 张表，
-- 探针回来发现喂不进数据，就得再写一轮迁移把它们改掉。
-- 建哪些表、以什么口径建，裁定在 docs/MVP裁定.md；列的取舍对着 source_probe_log 的
-- fields_seen 与 sample，探针没量到的字段不建列。
--
...
CREATE TABLE IF NOT EXISTS `source_probe_log` (
  ...
  `reachability` enum('direct','proxy','offline','blocked','error') NOT NULL DEFAULT 'error' COMMENT 'direct/proxy/offline/blocked/error 而非布尔：本机直连 Wikimedia 全线超时，无人值守的 scheduler 不能靠临时代理；offline 表示这一趟是用 data/raw 归档重放的，没联网，不能当成可达性证据',
...
-- ============================================================
-- 业务表（P1 的 C1c）。以下 DDL 与 db/migrations/0002_business_tables.sql 是同一份内容，
-- 改结构要两边一起改：那边负责已有库增量前进，这份负责全新库一次建齐。
-- ============================================================
