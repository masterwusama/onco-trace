...
这三条在只读的那一侧（D3a 起的 `api/onco_api/`）各有各的落点，页面不必自己再去对账：
...
2. 哨兵值不写 NULL 的读法：页面上"中国国家级单点"与"全分期头条"这两个档读的是 `year = 0`
   而不是 `year IS NULL`，所以十个维的度量都按 §一 那句口径写成对哨兵值的等式（`dimensions.py`）。
3. 建而不填的那些列在页面上是谓词而不是抄来的文案：`gaps.py` 读的就是逐维度量，`paf == 0` 才报
